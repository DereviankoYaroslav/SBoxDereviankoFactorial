...
int main(int args, char **argv) {
    ...
    //int ar [] = {239, 165, 56, 107, 229, 59, 66, 143, 81, 166, 241, 161, 140, 97, 36, 183, 153, 240, 225, 220, 87, 203, 139, 43, 54, 32, 253, 124, 73, 119, 171, 93, 212, 168, 83, 238, 222, 127, 60, 223, 207, 77, 174, 248, 19, 245, 12, 147, 106, 167, 164, 205, 187, 86, 69, 219, 128, 65, 1, 74, 75, 180, 172, 13, 188, 71, 197, 226, 150, 201, 3, 76, 244, 52, 155, 88, 39, 191, 125, 148, 228, 154, 170, 61, 53, 137, 158, 37, 9, 21, 94, 221, 123, 185, 218, 22, 92, 28, 210, 68, 217, 251, 50, 199, 101, 64, 115, 55, 252, 255, 200, 178, 176, 79, 169, 152, 67, 78, 216, 49, 129, 51, 177, 30, 105, 29, 85, 18, 202, 23, 63, 190, 195, 108, 198, 62, 136, 27, 130, 254, 109, 141, 116, 189, 26, 91, 102, 213, 111, 237, 120, 84, 231, 232, 90, 193, 151, 159, 4, 10, 38, 100, 48, 33, 132, 184, 214, 99, 34, 57, 234, 17, 156, 250, 98, 204, 15, 80, 126, 41, 181, 209, 45, 162, 95, 227, 131, 233, 8, 134, 243, 236, 122, 149, 104, 242, 194, 5, 0, 247, 89, 145, 114, 40, 24, 35, 175, 246, 249, 103, 72, 112, 135, 96, 117, 215, 6, 206, 42, 230, 47, 173, 44, 46, 118, 31, 7, 113, 163, 192, 82, 235, 208, 70, 133, 58, 160, 2, 11, 186, 14, 179, 142, 211, 144, 182, 16, 196, 138, 157, 224, 121, 110, 20, 25, 146};
    //1  int sbox [] = {10, 9, 13, 6, 14, 11, 4, 5, 15, 1, 3, 12, 7, 0, 8, 2};
    //2  int sbox [] = {8, 0, 12, 4, 9, 6, 7, 11, 2, 3, 1, 15, 5, 14, 10, 13};
    //3  int sbox [] = {15, 6, 5, 8, 14, 11, 10, 4, 12, 0, 3, 7, 2, 9, 1, 13};
    //4  int sbox [] = {3, 8, 13, 9, 6, 11, 15, 0, 2, 5, 12, 10, 4, 14, 1, 7};
    //5  int sbox [] = {15, 8, 14, 9, 7, 2, 0, 13, 12, 6, 1, 5, 11, 4, 3, 10};
    //6  int sbox [] = {2, 8, 9, 7, 5, 15, 0, 11, 12, 1, 13, 14, 10, 3, 6, 4};
    //7  int sbox [] = {3, 8, 11, 5, 6, 4, 14, 10, 2, 12, 1, 7, 9, 15, 13, 0};
    int sbox [] = {1, 2, 3, 14, 6, 13, 11, 8, 15, 10, 12, 5, 7, 9, 0, 4};

    //int *ar = SBoxToBooleanFunc(sbox, 16, 4);
    /*int nl = NLOfSBox(ar,16,4);
    printf("nl = %d", nl);
    long long cost = costFunctionCubaLinComb(sbox, 16, 4);
    printf("cost = %lld", cost);*/

    n = 16;

    //int sub1 [] = {8, 0, 12, 4, 9, 6, 7, 11, 2, 3, 1, 15, 5, 14, 10, 13};

    int *sub2 = calloc(n, sizeof(int));
    for (int r = 0; r < 16; ++r){
        sub2[r] = sbox[r]+1;
    }
...
//Функція перетворення факторіального числа на перестановку
...
//Функція перетворення десяткового числа на факторіальне число
...
//Функція перетворення перестановки на факторіальне число
...
//Функція перетворення факторіального числа на десяткове число
    ...
    while(maxIter > 0) {
        ...
        for (int i = 0; i < N; ++i) {
            for (int j = 0; j < size;) {
                ...
                if (i < ceil(N/2)-1) {
                    ...
                    X = myModulusDec((population[i][j] + Vel[i][j]), 256);
                }
                int contains;
                if (contains == 0) {
                    tempSbox[j] = X;
            ...
            if (i < ceil(N/2)-1) {
                ...
                if (NL > 98) {
                    int fl = 0;
                    for (int v = 0; v < 15; ++v) {
                        srand(tempSbox[v] * (curIter+fl * v) % 256);
                        if (i == 0){
                            int coeff = rand() % 50;
        ...
        for (int h = 1; h < N; ++h){
            ...
            if (flag102 == 0 && NL3 == 102 && ai3 == 3 && lr == 0){
                FILE *fileLocal;
                fopen_s(&fileLocal, "..\\Modified PSO\\N = 40 MaxIter = 200\\Table results N, MaxIter, IterToFind, Time.txt", "a");
                if (fileLocal == NULL) {
                    printf("ERROR: Can't save sbox to file!\n");
                    for (;;);
                }
                ...
                fprintf(fileLocal, "\n%d       %d       %d              %d        %d,%d", NL3,N,200,curIter+1,(int)a,(int)((-1)*(floor(a)-a)*1000000));
                printf("current iter = %d ",curIter+1);
                fclose(fileLocal);
                flag102 = 1;
            }
